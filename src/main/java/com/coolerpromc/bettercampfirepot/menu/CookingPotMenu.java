...
public class CookingPotMenu extends AbstractContainerMenu {
    ...
    public CookingPotMenu(int containerId, Inventory playerInventory, BlockPos blockPos) {
        this(containerId, playerInventory, (BetterCampfireBlockEntity) playerInventory.player.level().getBlockEntity(blockPos), new SimpleContainerData(4));
    }
    ...
    public CookingPotMenu(int containerId, Inventory playerInventory, BetterCampfireBlockEntity blockEntity, ContainerData containerData){
        super(BetterCampfirePot.BETTER_CAMPFIRE_MENU, containerId);
        ...
        SimpleContainer inputHandler = blockEntity.inputHandler;
        ...
        for (int i = 0; i < seasoningHandler.getContainerSize(); i++) {
            int finalI = i;
            addSlot(new Slot(seasoningHandler, finalI, 110 + i * 18, 18){
                @Override
                public boolean mayPlace(ItemStack itemStack) {
                    return seasoningHandler.canPlaceItem(finalI, itemStack);
                }
            });
        }

        SimpleContainer outputHandler = blockEntity.outputHandler;
        addSlot(new Slot(outputHandler, 0, 128, 55){
            @Override
            public boolean mayPlace(ItemStack itemStack) {
                return false;
            }
